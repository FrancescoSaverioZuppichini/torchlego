...
class ResNetBasicBlock(nn.Module):
    ...
    def __init__(self,
        ...
        super().__init__()
        shortcut = conv_bn(
            in_features, out_features, kernel_size=1, stride=2,
        ...
        self.blocks = nn.Sequential(
            ResidualAdd(nn.ModuleList([
                nn.Sequential(
                    conv3x3_bn_act(in_features,
                                   out_features,
                                   stride=stride,
                                   padding=1,
                                   bias=False,
                                   act=act),
                    conv3x3_bn(out_features,
                               out_features,
                               padding=1,
                               bias=False))
            ]),
                shortcut=shortcut), act())
...
class ResNetBottleNeck(nn.Module):
    ...
    def __init__(self,
        ...
        self.blocks = nn.Sequential(
            ResidualAdd(nn.ModuleList([
                nn.Sequential(
                    conv_bn_act(in_features,
                                out_features,
                                kernel_size=1,
                                stride=stride,
                                bias=False,
                                act=act),
                    conv3x3_bn_act(out_features,
                                   out_features,
                                   padding=1,
                                   bias=False,
                                   act=act)),
                conv_bn(out_features,
                        out_features * self.expansion,
                        kernel_size=1,
                        bias=False)
            ]),
...
class ResNet(VisionModule):
    def __init__(self,
        ...
        self.encoder = nn.Sequential(
            ...
            nn.Sequential(*[block(64, 64) for _ in range(n_blocks[0])]),
            block(64 * block.expansion, 128),
            nn.Sequential(*[block(128 * block.expansion, 128) for _ in range(n_blocks[1] - 1)]),
            block(128 * block.expansion, 256),
            nn.Sequential(*[block(256 * block.expansion, 256) for _ in range(n_blocks[2] - 1)]),
            block(256 * block.expansion,  512),
            nn.Sequential(*[block(512 * block.expansion, 512) for _ in range(n_blocks[3] - 1)]),
            nn.AdaptiveAvgPool2d(output_size=(1, 1)))
        self.decoder = nn.Sequential(nn.Flatten(), nn.Linear(512, n_classes))
